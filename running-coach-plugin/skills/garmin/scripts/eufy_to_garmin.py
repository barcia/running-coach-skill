#!/usr/bin/env python3
"""
Conversor de datos de báscula Eufy a formato Garmin Connect.

Uso:
    python eufy_to_garmin.py <archivo_eufy.csv> [--dry-run]

Este script lee un CSV exportado de la app EufyLife y genera JSON
compatible con el MCP de Garmin para subir composición corporal.
"""
...
PHYSIQUE_MAPPING = {
    ...
    "promedio": 5,
    "músculo estándar": 6,
    "musculo estándar": 6,  # Sin tilde en ú
    "músculo estandar": 6,   # Sin tilde en á
    "musculo estandar": 6,   # Sin tildes
    "delgado": 7,
...
# Mapeo de nombres de columnas Eufy → parámetros Garmin
COLUMN_MAPPING = {
    "Hora": "date",
    "PESO (kg)": "weight",
    "IMC": "bmi",
    "% DE GRASA CORPORAL %": "percent_fat",
    "%!D(MISSING)E GRASA CORPORAL %": "percent_fat",  # Header corrupto en algunos exports
    "MASA MUSCULAR (kg)": "muscle_mass",
    "MB": "basal_met",
    "AGUA": "percent_hydration",
    "MASA ÓSEA (kg)": "bone_mass",
    "GRASA VISCERAL": "visceral_fat_rating",
    "EDAD DEL CUERPO": "metabolic_age",
    "TIPO DE CUERPO": "physique_rating",
}
...
def read_eufy_csv(file_path: Path) -> list[dict]:
    ...
    # Intentar diferentes encodings (utf-8-sig primero para manejar BOM)
    ...
    for encoding in encodings:
        try:
            ...
            # Reabrir para leer con csv.DictReader
            with open(file_path, "r", encoding=encoding) as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="Convierte datos de báscula Eufy a formato Garmin Connect"
    )
    parser.add_argument(
        "csv_file",
        type=Path,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Solo mostrar datos convertidos, no subir a Garmin"
    )
    parser.add_argument(
        "--output",
        "-o",
        type=Path,
        help="Guardar JSON en archivo en lugar de imprimir"
    )
    parser.add_argument(
        "--latest",
        "-l",
    ...
    if not measurements:
        print("No se encontraron mediciones válidas en el archivo", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Encontradas {len(measurements)} mediciones válidas")

    if args.dry_run or args.output:
        # Mostrar o guardar JSON
        output = {
            "source": "eufy",
        ...
        # Instrucciones para subir via MCP
        print("\nPara subir a Garmin, usa el MCP con cada medición:")
        print("-" * 50)
